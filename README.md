...
A escala pessoal da semana pode chegar por link, sem digitação: um endereço
terminado em `#escala=<base64>` traz os dias e entra por cima do que já está
guardado, sem tocar no que foi marcado. O que vem depois do `#` fica no
aparelho e nunca é enviado ao servidor, então nome de colega não trafega. Um
turno pode ser `"livre"` — a dupla é outra —, e aí a agenda mostra quem está
na UBS e o turno conta como tempo de estudo.
